...
#### データ構造の変更
- `sticker_labels`テーブルに`year`（年）と`month`（月）カラムを追加
- ユニーク制約: `user_id` + `year` + `month` + `color`
- 既存データは現在の月（2025年10月）のラベルとして扱う
...
3. **データ移行**
   - 既存のラベルデータを2025年10月のデータとして扱う
   - LocalStorageとSupabase両方の移行処理を実装

#### 影響を受けるファイル
- `types/database.ts` - 型定義の更新（year, month追加）
- `lib/supabase/database.ts` - CRUD操作の更新（月別取得・保存）
- `hooks/useHybridStickerLabels.ts` - 月別ラベル取得ロジック追加
- `app/page.tsx` - 月変更時のラベル再取得処理
- Supabaseマイグレーション - テーブル構造変更スクリプト
