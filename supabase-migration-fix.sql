...
-- 注意: カスタム認証システムではauth.uid()は使用できないため、一時的に無効化
-- 実際の運用では適切な認証ポリシーを設定する必要があります

-- 一時的にすべてのユーザーがアクセス可能にする（開発用）
CREATE POLICY "Allow all access for custom auth" ON users FOR ALL USING (true);
CREATE POLICY "Allow all access for custom auth stickers" ON user_stickers FOR ALL USING (true);
CREATE POLICY "Allow all access for custom auth labels" ON user_sticker_labels FOR ALL USING (true);
